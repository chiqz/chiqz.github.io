...
var containerId = '#gameContainer', scoreContainerId = '#scoreContainer', lineCountContainerId = '#lineCountContainer', levelContainerId = '#levelContainer', piecePreviewId = '#piecePreviewContainer', pauseId = '#pauseContainer'
...
var displayBox = $(containerId), scoreBox = $(scoreContainerId), lineCountBox = $(lineCountContainerId), levelBox = $(levelContainerId), piecePreviewBox = $(piecePreviewContainer), pauseBox = $(pauseId)
  , matrixState = [], score = 0, lineCount = 0, currentLevel = 1, intervalId, gamePaused = false, gameUnpausing = false, gameFinished = false, inBlockZone = false, currentSpeed = initialSpeed, i, j;
...
var initializeGame = function() {
  ...
  pauseBox.css('left', leftZero + matrixWidth * minoWidth + HUDLeftOffset);
  ...
  pauseBox.css('top', topZero + HUDTopOffset + 5 * HUDLineOffset);
...
function unpauseCountdown(i) {
  if (i <= 0) {
    ...
    pauseBox.html(i + ' ...');
    setTimeout(function () { unpauseCountdown(i - 1); }, 600);
  }
}
